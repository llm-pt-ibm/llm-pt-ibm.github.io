...
- Usaremos FastAPI e Transformers para desenvolver essa API e HuggingFace para baixar os modelos.
...
Vamos usar FastAPI e Transformers para implementar a API. Além disso, usaremos uvicorn para executar o servidor, pydantic para validação de dados de entrada e torch, que já instalamos no [tutorial anterior]({{< relref "tutorial_power9_pt2.md" >}}).
...
- `load_model` carrega o modelo especificado. Caso o modelo necessite de um token HuggingFace, a função também recebe esse parâmetro.
...
Seguindo estas implementações, você terá uma API capaz de realizar inferências com Modelos de Linguagem baixados do HuggingFace. No [próximo tutorial]({{< relref "tutorial_power9_pt4.md" >}}) será demonstrado como enviar requisições para a API via curl e python.  