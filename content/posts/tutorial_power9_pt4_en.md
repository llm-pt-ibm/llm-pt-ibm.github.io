...
tags: ["LLM", "Power9", "API"]
summary: "This post is part of a tutorial series whose ultimate goal is to build a LLM API on a Power9 server. In this stage, we will present the API and show how to make requests."
...
First, we need to create a header containing the API Key returned from the code above and the payload with `model_name` and the HuggingFace token `hf_token`. After that, we can send the request with these two pieces of information.
